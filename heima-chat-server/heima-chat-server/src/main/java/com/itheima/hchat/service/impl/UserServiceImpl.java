   ...
   // private FastDFSClient fastDFSClient;
    ...
    public User upload(MultipartFile file, String userid) {
        try {
           ...
           // todo  String url = fastDFSClient.uploadFace(file);
            // 在FastDFS上传的时候，会自动生成一个缩略图
            // 文件名_150x150.后缀
           // todo  String[] fileNameList = url.split("\\.");
            // todo  String fileName = fileNameList[0];
           // todo  String ext = fileNameList[1];

            // todo String picSmallUrl = fileName + "_150x150." + ext;

           // String prefix = env.getProperty("fdfs.httpurl");
            String prefix = "www.baidu.com";
            TbUser tbUser = userMapper.selectByPrimaryKey(userid);

            // 设置头像大图片
            // todo tbUser.setPicNormal(prefix + url);
            // 设置头像小图片
            // todo tbUser.setPicSmall(prefix + picSmallUrl);
            // 将新的头像URL更新到数据库中
